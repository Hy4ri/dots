...
get_disk_usage() {
    ...
    echo "$DISK_INFO" | rofi -dmenu -p "System Stats" -theme "${dir}/${theme}.rasi" -no-fixed-num-lines
}
...
show_all_stats() {
    ...
    # Build output for rofi
    {
        ...
        echo "💾 Storage:"
        echo "$DISK_INFO"
    } | rofi -dmenu -p "System Stats" -theme "${dir}/${theme}.rasi" -no-fixed-num-lines
}
...
CHOICE=$(for item in "${MENU_ITEMS[@]}"; do
    ...
    echo "$label"
done | rofi -dmenu -p "System Monitor" -theme "${dir}/${theme}.rasi")
