{
  lib,
  config,
  ...
}: {
  xdg = {
    mime = {
      enable = true;
      defaultApplications = {
        "x-scheme-handler/terminal" = "foot.desktop";
        "text/html" = "vivaldi-stable.desktop";
        "x-scheme-handler/http" = "vivaldi-stable.desktop";
        "x-scheme-handler/https" = "vivaldi-stable.desktop";
        "x-scheme-handler/about" = "vivaldi-stable.desktop";
        "x-scheme-handler/unknown" = "vivaldi-stable.desktop";
        "x-scheme-handler/discord" = "equibop.desktop";
        "video/mpeg" = "mpv.desktop";
        "video/vnd.rn-realvideo" = "mpv.desktop";
        "video/3gpp2" = "mpv.desktop";
        "application/smil+xml" = "mpv.desktop";
        "audio/flac" = "mpv.desktop";
        "video/vnd.mpegurl" = "mpv.desktop";
        "video/x-flic" = "mpv.desktop";
        "audio/x-ms-asx" = "mpv.desktop";
        "audio/x-matroska" = "mpv.desktop";
        "application/mxf" = "mpv.desktop";
        "audio/x-ape" = "mpv.desktop";
        "video/x-matroska" = "mpv.desktop";
        "audio/x-ms-wma" = "mpv.desktop";
        "audio/aac" = "mpv.desktop";
        "video/ogg" = "mpv.desktop";
        "application/vnd.ms-asf" = "mpv.desktop";
        "video/mp2t" = "mpv.desktop";
        "audio/x-tta" = "mpv.desktop";
        "audio/x-musepack" = "mpv.desktop";
        "audio/x-vorbis+ogg" = "mpv.desktop";
        "video/mp4" = "mpv.desktop";
        "audio/ogg" = "mpv.desktop";
        "video/vnd.avi" = "mpv.desktop";
        "audio/x-adpcm" = "mpv.desktop";
        "application/sdp" = "mpv.desktop";
        "application/x-matroska" = "mpv.desktop";
        "audio/ac3" = "mpv.desktop";
        "audio/mp4" = "mpv.desktop";
        "audio/x-scpls" = "mpv.desktop";
        "audio/vnd.dts" = "mpv.desktop";
        "application/x-cue" = "mpv.desktop";
        "video/x-ogm+ogg" = "mpv.desktop";
        "audio/AMR" = "mpv.desktop";
        "video/quicktime" = "mpv.desktop";
        "video/x-ms-wmv" = "mpv.desktop";
        "audio/webm" = "mpv.desktop";
        "application/ogg" = "mpv.desktop";
        "video/webm" = "mpv.desktop";
        "audio/vnd.dts.hd" = "mpv.desktop";
        "audio/vnd.rn-realaudio" = "mpv.desktop";
        "audio/x-mpegurl" = "mpv.desktop";
        "application/vnd.apple.mpegurl" = "mpv.desktop";
        "video/dv" = "mpv.desktop";
        "audio/mpeg" = "mpv.desktop";
        "audio/x-wavpack" = "mpv.desktop";
        "audio/vnd.wave" = "mpv.desktop";
        "audio/mp2" = "mpv.desktop";
        "video/3gpp" = "mpv.desktop";
        "application/vnd.rn-realmedia" = "mpv.desktop";
        "video/x-theora+ogg" = "mpv.desktop";
        "audio/x-aiff" = "mpv.desktop";
        "video/x-flv" = "mpv.desktop";
        "application/x-shorten" = "mpv.desktop";
        "image/svg+xml" = "frame.desktop";
        "image/webp" = "frame.desktop";
        "image/heif" = "frame.desktop";
        "image/x-xbitmap" = "frame.desktop";
        "image/tiff" = "frame.desktop";
        "image/vnd.zbrush.pcx" = "frame.desktop";
        "image/x-portable-graymap" = "frame.desktop";
        "image/x-portable-pixmap" = "frame.desktop";
        "image/gif" = "frame.desktop";
        "image/bmp" = "frame.desktop";
        "image/jpeg" = "frame.desktop";
        "image/png" = "frame.desktop";
        "image/x-portable-bitmap" = "frame.desktop";
        "image/x-portable-anymap" = "frame.desktop";
        "image/x-tga" = "frame.desktop";
        "application/x-fictionbook+xml" = "org.pwmt.zathura.desktop";
        "application/vnd.comicbook+zip" = "org.pwmt.zathura.desktop";
        "application/epub+zip" = "org.pwmt.zathura.desktop";
        "application/pdf" = "org.pwmt.zathura.desktop";
        "application/oxps" = "org.pwmt.zathura.desktop";
        "application/vnd.ms-xpsdocument" = "org.pwmt.zathura.desktop";
        "text/x-moc" = "nvim.desktop";
        "text/x-pascal" = "nvim.desktop";
        "application/x-shellscript" = "nvim.desktop";
        "text/x-shellscript" = "nvim.desktop";
        "text/x-chdr" = "nvim.desktop";
        "text/x-tex" = "nvim.desktop";
        "text/x-c++hdr" = "nvim.desktop";
        "text/x-csrc" = "nvim.desktop";
        "text/x-makefile" = "nvim.desktop";
        "text/x-c++src" = "nvim.desktop";
        "text/plain" = "nvim.desktop";
        "text/tcl" = "nvim.desktop";
        "text/x-java" = "nvim.desktop";
      };
    };
  };
}
